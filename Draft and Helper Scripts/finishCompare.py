...
oDir = './Matching Output'
if not os.path.isdir(oDir) or not os.path.exists(oDir):
    os.makedirs(oDir)


all_bn_df = pd.read_excel(pDir + "/All Barnes & Noble - Separate Rows for Each ISBN - Fall 2019.xlsx")

all_bn_df['ISBN'] = all_bn_df['ISBN'].apply(lambda x: smart_str(x))

matching_bn_df = pd.read_excel(pDir + '/Initial Matched Alma ISBNs - All.xlsx', dtype={'MMS_ID': str})
...
master = pd.merge(all_bn_df, csdf, on=['Course'], how='inner')
...
master = pd.merge(master, matching_bn_df, on=['ISBN'], how='outer', indicator=True)
